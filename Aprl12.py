...
def sim():
    ...
    plt.bar(labels, counts, color=["pink", "Green"])
    
    plt.show()

    labels = ['Influenza', 'Common Cold', 'Eczema', 'Asthma ' ,'Hyperthyroidism']
    sizes = [50,40, 30, 20]
    plt.figure(figsize=(8, 5))
    plt.pie(sizes, labels=labels, autopct='%1.1f%%', colors=['pink', 'blue', 'red', 'green'])
    plt.title('Pie Chart')
    plt.show()
